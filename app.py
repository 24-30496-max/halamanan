...
def home():
    ...
    if connection:
        ...
        cursor.execute("SELECT COUNT(*) as count FROM resources WHERE status = 'Available'")
        total_resources = cursor.fetchone()['count']
        
        cursor.execute("SELECT COUNT(*) as count FROM requests WHERE status = 'Available'")
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if check_password_hash(user["password_hash"], password):
                user_obj = User(user['user_id'], user['username'], user['email'], user['phone'], user['province'], user['city'])
                login_user(user_obj)
            ...
            flash("No account found with that email or username. Please register first.", "error")
    ...
    if request.method == 'POST':
        ...
        if connection:
            try:
                ...
                """, (name, email, phone, province, city, hashed_password, datetime.now()))
    ...
    if request.method == 'POST':
        ...
        if connection:
            try:
                ...
                """, (current_user.id, category, name, description, 
                      start_available_date, end_available_date, 'Available', datetime.now()))
        ...
        if connection:
            try:
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (current_user.id, category, needed_item, description, 'Available', datetime.now()))
...
def profile():
    ...
    if connection:
        ...
        # Get user's requests
        cursor.execute("SELECT * FROM requests WHERE user_id = %s ORDER BY created_at DESC", (user_id,))
...
def inquire_resource(resource_id):
    ...
    if connection:
        try:
            ...
            # Get resource info
            cursor.execute("SELECT user_id FROM resources WHERE resource_id = %s", (resource_id,))
            resource = cursor.fetchone()
...
def pending_payments():
    ...
    if connection:
        ...
        try:
            ...
            return render_template('seller_payments.html',
    ...
    if connection:
        try:
            ...
            cursor.execute("""
                ...
                WHERE payment_id = %s
            """, ('Failed', current_user.id, datetime.now(), rejection_reason, payment_id))
...
def payment(resource_id):
    ...
    if connection:
        ...
        if request.method == 'POST':
            ...
            transaction_ref = request.form.get('transaction_ref')  # NEW: Get transaction ref
            ...
            try:
                ...
                """, (resource_id, current_user.id, resource['user_id'], amount, 
                      payment_method, 'Pending', transaction_ref, notes, datetime.now()))
...
def edit_offer(resource_id):
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    cursor.execute("SELECT * FROM resources WHERE resource_id=%s AND user_id=%s", (resource_id, current_user.id))
    ...
    if request.method == 'POST':
        ...
        """, (category, name, description, start_available_date, end_available_date, arrangement, resource_type, resource_id, current_user.id))
        connection.commit()
...
def edit_request(request_id):
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    cursor.execute("SELECT * FROM requests WHERE request_id=%s AND user_id=%s", (request_id, current_user.id))
    req = cursor.fetchone()
    if not req:
    ...
    if request.method == 'POST':
        ...
        """, (category, needed_item, description, needed_date, urgency, arrangement, request_id, current_user.id))
        connection.commit()
